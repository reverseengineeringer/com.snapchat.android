  ...
  implements Parcelable
{
  ...
  {
    private static BackStackState createFromParcel(Parcel paramAnonymousParcel)
    {
      return new BackStackState(paramAnonymousParcel);
    }
    
    private static BackStackState[] newArray(int paramAnonymousInt)
    {
  ...
  public final BackStackRecord instantiate(FragmentManagerImpl paramFragmentManagerImpl)
  {
    ...
    while (i < mOps.length)
    {
      ...
      if (FragmentManagerImpl.DEBUG) {
        new StringBuilder("Instantiate ").append(localBackStackRecord).append(" op #").append(k).append(" base fragment #").append(mOps[j]);
      }
      localObject = mOps;
      i = j + 1;
      j = localObject[j];
      ...
      for (fragment = ((Fragment)mActive.get(j));; fragment = null)
      {
        ...
        for (;;)
        {
          ...
          if (FragmentManagerImpl.DEBUG) {
            new StringBuilder("Instantiate ").append(localBackStackRecord).append(" set remove fragment #").append(mOps[j]);
          }
