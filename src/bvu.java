import in.srain.cube.views.ptr.PtrFrameLayout;

public final class bvu
  implements bvt
{
  public bvt a;
  public bvu b;
  
  public final void a(PtrFrameLayout paramPtrFrameLayout)
  {
    Object localObject1 = this;
    Object localObject2;
    do
    {
      localObject2 = a;
      if (localObject2 != null) {
        ((bvt)localObject2).a(paramPtrFrameLayout);
      }
      localObject2 = b;
      localObject1 = localObject2;
    } while (localObject2 != null);
  }
  
  public final void a(PtrFrameLayout paramPtrFrameLayout, boolean paramBoolean, byte paramByte, int paramInt1, int paramInt2, float paramFloat1, float paramFloat2)
  {
    Object localObject1 = this;
    Object localObject2;
    do
    {
      localObject2 = a;
      if (localObject2 != null) {
        ((bvt)localObject2).a(paramPtrFrameLayout, paramBoolean, paramByte, paramInt1, paramInt2, paramFloat1, paramFloat2);
      }
      localObject2 = b;
      localObject1 = localObject2;
    } while (localObject2 != null);
  }
  
  public final boolean a()
  {
    return a != null;
  }
  
  public final void b(PtrFrameLayout paramPtrFrameLayout)
  {
    Object localObject1 = this;
    Object localObject2;
    do
    {
      localObject2 = a;
      if (localObject2 != null) {
        ((bvt)localObject2).b(paramPtrFrameLayout);
      }
      localObject2 = b;
      localObject1 = localObject2;
    } while (localObject2 != null);
  }
  
  public final void c(PtrFrameLayout paramPtrFrameLayout)
  {
    Object localObject1 = this;
    Object localObject2;
    do
    {
      localObject2 = a;
      if (localObject2 != null) {
        ((bvt)localObject2).c(paramPtrFrameLayout);
      }
      localObject2 = b;
      localObject1 = localObject2;
    } while (localObject2 != null);
  }
  
  public final void d(PtrFrameLayout paramPtrFrameLayout)
  {
    Object localObject1 = this;
    Object localObject2;
    do
    {
      localObject2 = a;
      if (localObject2 != null) {
        ((bvt)localObject2).d(paramPtrFrameLayout);
      }
      localObject2 = b;
      localObject1 = localObject2;
    } while (localObject2 != null);
  }
}

/* Location:
 * Qualified Name:     bvu
 * Java Class Version: 6 (50.0)
 * JD-Core Version:    0.7.1
 */